...
class FaceWapper():
    ...
    def __init__(self,headImage,faceImage,predictor_path="./data/shape_predictor_68_face_landmarks.dat"):
        #五官等标记点
        self.SCALE_FACTOR = 1 
        ...
        im1, landmarks1 = self.read_im_and_landmarks(headImage)
        im2, landmarks2 = self.read_im_and_landmarks(faceImage)

        M = self.transformation_from_points(landmarks1[self.ALIGN_POINTS],landmarks2[self.ALIGN_POINTS])
        ...
        self.output_im = im1 * (1.0 - combined_mask) + warped_corrected_im2 * combined_mask

        # cv2.imwrite('output.jpg', output_im)
    def uuid_image_name(self):
        return str(uuid.uuid1().time) + ".jpg"

    def save_image_special(self,path):
        self.save(path,self.output_im)

    def read_im_and_landmarks(self,fname):
        im = cv2.imread(fname, cv2.IMREAD_COLOR)
        im = cv2.resize(im, (im.shape[1] * self.SCALE_FACTOR,
                            im.shape[0] * self.SCALE_FACTOR))
        s = self.get_landmarks(im,fname)
...
if __name__ == "__main__":
    fa = FaceWapper(sys.argv[1],sys.argv[2])
    fa.save_image_special('/Users/godfery/Downloads/godfery.jpg')